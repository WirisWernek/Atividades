...
int repete()
{
	printf("Insira o numero de herois que voc� quer cadastrar\n");
	scanf("%i", &Repete);
...
int poder()
{
	...
	{
		printf("Her�i %i\n", i + 1);
		printf("Qual tipo de poder voc� quer para seu her�i?\nGelo\nFogo\nAgua\nTerra\nAr\nDigite seu poder: ");
		scanf("%s", &ponteiro[i]->Poder.TipoDePoder);
		qualpoder();

		printf("\nInsira um nome para o poder do seu Her�i: ");
...
int imprime()
{
	setlocale(LC_ALL, "Portuguese");
	printf("Her�i %i\n", i + 1);
	printf("O nome do her�i �: %s\n", Super[i].NomeHeroi);
	printf("O nome do poder �: %s\n", Super[i].Poder.NomePoder);
	printf("O tipo de poder �: %s\n", Super[i].Poder.TipoDePoder);
	printf("Esse poder tem %i de Dano\n", Super[i].Poder.Dano);
	printf("Seu her�i tem %i de HP\n", Super[i].HP);
